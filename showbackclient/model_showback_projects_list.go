/*
Taikun - Showback API

This Api will be responsible for overall data distribution and authorization.

API version: v1
Contact: noreply@itera.io
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package taikunshowback

import (
	"encoding/json"
)

// checks if the ShowbackProjectsList type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ShowbackProjectsList{}

// ShowbackProjectsList struct for ShowbackProjectsList
type ShowbackProjectsList struct {
	Data []ShowbackProjectsListDto `json:"data,omitempty"`
	TotalCount *int32 `json:"totalCount,omitempty"`
}

// NewShowbackProjectsList instantiates a new ShowbackProjectsList object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewShowbackProjectsList() *ShowbackProjectsList {
	this := ShowbackProjectsList{}
	return &this
}

// NewShowbackProjectsListWithDefaults instantiates a new ShowbackProjectsList object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewShowbackProjectsListWithDefaults() *ShowbackProjectsList {
	this := ShowbackProjectsList{}
	return &this
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *ShowbackProjectsList) GetData() []ShowbackProjectsListDto {
	if o == nil || IsNil(o.Data) {
		var ret []ShowbackProjectsListDto
		return ret
	}
	return o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ShowbackProjectsList) GetDataOk() ([]ShowbackProjectsListDto, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *ShowbackProjectsList) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given []ShowbackProjectsListDto and assigns it to the Data field.
func (o *ShowbackProjectsList) SetData(v []ShowbackProjectsListDto) {
	o.Data = v
}

// GetTotalCount returns the TotalCount field value if set, zero value otherwise.
func (o *ShowbackProjectsList) GetTotalCount() int32 {
	if o == nil || IsNil(o.TotalCount) {
		var ret int32
		return ret
	}
	return *o.TotalCount
}

// GetTotalCountOk returns a tuple with the TotalCount field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ShowbackProjectsList) GetTotalCountOk() (*int32, bool) {
	if o == nil || IsNil(o.TotalCount) {
		return nil, false
	}
	return o.TotalCount, true
}

// HasTotalCount returns a boolean if a field has been set.
func (o *ShowbackProjectsList) HasTotalCount() bool {
	if o != nil && !IsNil(o.TotalCount) {
		return true
	}

	return false
}

// SetTotalCount gets a reference to the given int32 and assigns it to the TotalCount field.
func (o *ShowbackProjectsList) SetTotalCount(v int32) {
	o.TotalCount = &v
}

func (o ShowbackProjectsList) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ShowbackProjectsList) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	if !IsNil(o.TotalCount) {
		toSerialize["totalCount"] = o.TotalCount
	}
	return toSerialize, nil
}

type NullableShowbackProjectsList struct {
	value *ShowbackProjectsList
	isSet bool
}

func (v NullableShowbackProjectsList) Get() *ShowbackProjectsList {
	return v.value
}

func (v *NullableShowbackProjectsList) Set(val *ShowbackProjectsList) {
	v.value = val
	v.isSet = true
}

func (v NullableShowbackProjectsList) IsSet() bool {
	return v.isSet
}

func (v *NullableShowbackProjectsList) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableShowbackProjectsList(val *ShowbackProjectsList) *NullableShowbackProjectsList {
	return &NullableShowbackProjectsList{value: val, isSet: true}
}

func (v NullableShowbackProjectsList) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableShowbackProjectsList) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


