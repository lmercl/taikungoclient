...
func (r ApiRepositoryDisableRequest) DisableRepoCommand(disableRepoCommand DisableRepoCommand) ApiRepositoryDisableRequest {
	r.disableRepoCommand = &disableRepoCommand
	return r
}
...
func (a *AppRepositoriesApiService) RepositoryDisableExecute(r ApiRepositoryDisableRequest) (*http.Response, error) {
	...
	localVarPath := localBasePath + "/api/v1/repository/disable"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.disableRepoCommand == nil {
		return nil, reportError("disableRepoCommand is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.disableRepoCommand
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["Bearer"]; ok {
